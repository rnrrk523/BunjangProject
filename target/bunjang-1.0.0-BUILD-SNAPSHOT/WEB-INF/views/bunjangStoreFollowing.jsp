...
<script>
		$(function() {
			...
			$(document).on("click", ".Store_follow_Btn", function() {
				let targetStoreIdx = $(this).data("idx");
				let btn = $(this);
				let sessionIdx = "${storeIdx}";
				if(sessionIdx != targetStoreIdx) {
					$.ajax({
						type: "POST",
				        url: 'ajaxFollow',
				        data: {
				            "storeIdx":"${storeIdx}",
				            "viewStoreIdx":targetStoreIdx
				        },
				        dataType: "text",
				        success: function(data) {
				        	btn.empty();
				        	console.log(data);
				        	if(data == "on") {
				        		let btnIn = "<img src=\"${pageContext.request.contextPath}/resources/images/following.png\" width=\"20\" height=\"14\" alt=\"화이트 테마 언팔로우 버튼 아이콘\">팔로잉";
				        		btn.append(btnIn);
				        		btn.addClass("on");
				        	}else if(data == "off") {
				        		let btnIn = "<img src=\"${pageContext.request.contextPath}/resources/images/following_off.png\" width=\"20\" height=\"14\" alt=\"화이트 테마 언팔로우 버튼 아이콘\">팔로우";
				        		btn.append(btnIn);
				        		btn.removeClass("on");
				        	}
				        },
					...
					<div id="a2">
						<c:choose>
							<c:when test="${jjimCnt > 0}">
								<a id="a3" href="${pageContext.request.contextPath}/Storejjim?viewStoreIdx=${storeIdx}" class="on"><img src="https://m.bunjang.co.kr/pc-static/resource/f23127455135173fd3e9.png" width="12" height="12" alt="찜 아이콘" style="margin-right: 5px;"/><span id="favorites_cnt">${jjimCnt}</span></a>
							</c:when>
							<c:otherwise>
		...
		<div class = "main_area">
			...
			<div>
				<div class = "Store_nav">
					...
					<a class = "Store_nav_category on" href="${pageContext.request.contextPath}/StoreFollowing?viewStoreIdx=${viewStoreIdx}">
						팔로잉
			...
			<div>
				<div>
					<div class = "Store_product_header">
						<div>
							팔로잉
							<span class = "Product_number">${store.followingCount}</span>
						</div>
					</div>
					<c:choose>
					  <c:when test="${store.followingCount == 0}">
						<div class = "Store_product_null">아직 팔로우한 사람이 없습니다.</div>
						<div class = "Store_following_content" style = "display:none">
					  </c:when>
					  <c:otherwise>
						<div class = "Store_product_null" style = "display:none">아직 팔로우한 사람이 없습니다.</div>
						<div class = "Store_following_content">
					  </c:otherwise>
					</c:choose>
					<c:forEach var="following" items="${followingList}">
						<div class = "Store_following_item" data-Sno = "${following.storeIdx}">
							<div class = "Store_following_item_box">
								<a class = "Store_following_link" href="${pageContext.request.contextPath}/StoreProduct?viewStoreIdx=${following.storeIdx}">
									<c:choose>
								      <c:when test="${empty following.profileImg}">
								        <img
								      ...
								      <c:otherwise>
								        <img src="${following.profileImg}"
								             width="100" height="100"
								             alt="${following.storeName} 프로필"
								             class="Store_Profile_img" />
								      </c:otherwise>
								    </c:choose>
								</a>
								<a class = "Store_following_name" href="${pageContext.request.contextPath}/StoreProduct?viewStoreIdx=${following.storeIdx}">${following.storeName}</a>
								<div class = "Store_follow_count_box">
									<a class = "Store_Product_few_link">
										상품
										<b>${following.productCount}</b>
									</a>
									<a class = "Store_follower_few_link">
										팔로워
										<b>${following.followerCount}</b>
									</a>
								</div>
								<div class = "Store_follow_Btn_Box">
									<c:choose>
										<c:when test="${following.followingYN eq \"Y\"}">
											<button class = "Store_follow_Btn on" data-idx="${following.storeIdx}">
												<img src="${pageContext.request.contextPath}/resources/images/following.png" width="20" height="14" alt="화이트 테마 언팔로우 버튼 아이콘">
												팔로잉
											</button>
										</c:when>
										<c:otherwise>
											<button class = "Store_follow_Btn" data-idx="${following.storeIdx}">
												<img src="${pageContext.request.contextPath}/resources/images/following_off.png" width="20" height="14" alt="화이트 테마 언팔로우 버튼 아이콘">
												팔로우
											</button>
										</c:otherwise>
									</c:choose>
							...
							<div class = "Store_following_item_line"></div>
						</div>
					</c:forEach>
					</div>
