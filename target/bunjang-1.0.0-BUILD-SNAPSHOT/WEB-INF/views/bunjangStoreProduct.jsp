...
복구되지 않습니다.</div>
	...
	<div class ="main_body">
		<div class = "main_area">
			...
			<div>
				<div class = "Store_nav">
					<a class = "Store_nav_category on" href="${pageContext.request.contextPath}/StoreProduct?viewStoreIdx=${viewStoreIdx}">
						상품
			...
			<div>
				<div>
					<div class = "Store_product_header">
						<div>
							상품
							<span class = "Product_number">${store.productCount}</span>
						</div>
						<div class = "Product_filter">
							<div class = "Product_filter_box">
								<div class = "Product_filter_box2">
									${selectedCategoryName}
									<img src="${pageContext.request.contextPath}/resources/images/category_arrow2.png" width="10" height="6" alt="카테고리 화살표 아이콘">
								</div>
								<div class = "Product_filter_category">
									<c:set var="strParam" value="${param.str != null && param.str != '' ? param.str : '최신순'}" />
									<a class="Product_filter_category2 ${selectedCategory == 0 ? 'on' : 'off'}" 
									   href="${pageContext.request.contextPath}/StoreProduct?viewStoreIdx=${store.storeIdx}&category=0&str=${strParam}">
									   전체
									</a>
									<c:forEach var="category" items="${categoryList}">
									    <a class="Product_filter_category2 ${selectedCategory == category.categoryIdx ? 'on' : 'off'}" 
									       href="${pageContext.request.contextPath}/StoreProduct?viewStoreIdx=${store.storeIdx}&category=${category.categoryIdx}&str=${strParam}">
					...
					<c:choose>
					  <c:when test="${store.productCount == 0}">
						<div class = "Store_product_null">등록된 상품이 없습니다.</div>
					  </c:when>
					  <c:otherwise>
						<div class = "Store_Follower_main">
							<div class = "Store_product_main_header">
								<div class = "Store_product_main_header_box">
									<div class = "Store_product_main_header_category">
										<div>${selectedCategoryName}</div>
										<div class = "Store_product_number">${productCount}개</div>
									</div>
									<div class = "Store_product_main_header_filter">
										<c:set var="strParam" value="${param.str != null && param.str != '' ? param.str : '최신순'}" />
									    <a class="Store_filter_this ${strParam == '최신순' ? 'on' : ''}" 
									       href="${pageContext.request.contextPath}/StoreProduct?viewStoreIdx=${store.storeIdx}&category=${selectedCategory}&str=최신순">
									       최신순
									    </a>
									    <a class="Store_filter_this ${strParam == '인기순' ? 'on' : ''}" 
									       href="${pageContext.request.contextPath}/StoreProduct?viewStoreIdx=${store.storeIdx}&category=${selectedCategory}&str=인기순">
									       인기순
									    </a>
									    <a class="Store_filter_this ${strParam == '저가순' ? 'on' : ''}" 
									       href="${pageContext.request.contextPath}/StoreProduct?viewStoreIdx=${store.storeIdx}&category=${selectedCategory}&str=저가순">
									       저가순
									    </a>
									    <a class="Store_filter_this ${strParam == '고가순' ? 'on' : ''}" 
									       href="${pageContext.request.contextPath}/StoreProduct?viewStoreIdx=${store.storeIdx}&category=${selectedCategory}&str=고가순">
									       고가순
									    </a>
									</div>
								</div>
							</div>
							<div class = "Store_product_main_content">
							<c:forEach var="product" items="${productList}">
								<div class = "Store_product_item" data-pno = "${product.productIdx}">
									<a class = "Store_product_item_link" href="${pageContext.request.contextPath}/productInfo?productIdx=${product.productIdx}">
										<div class = "Store_product_item_image">
											<c:choose>
										      <c:when test="${empty product.productImg}">
										        <img
										          src="data:image/svg+xml;base64,"
										          width="194" height="194"											          alt="상점 프로필 이미지"
										          />
										      </c:when>
										      <c:otherwise>
										...
										<div class = "Store_product_item_information">
											<div class = "Store_product_item_name">${product.productName}</div>
											<div class = "Store_product_item_information2">
												<div class = "Store_product_item_pay">${product.price}</div>
												<div class = "Store_product_item_newDate">
							...
							<div class = "Store_product_main_fotter"></div>
						</div>
					  </c:otherwise>
					</c:choose>
				</div>
