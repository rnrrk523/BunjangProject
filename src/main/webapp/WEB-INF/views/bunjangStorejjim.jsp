...
<script>
	$(document).ready(function() {
		$('.jjim_SelectALLbtn_box').click(function(e) {
		    e.preventDefault();

		    const isOn = $(this).hasClass('on');
		    const isInStoreHeader = $(this).closest('.Store_jjim_headers').length > 0;

		    if (isInStoreHeader) {
		        if (isOn) {
		            $('.jjim_SelectALLbtn_box').removeClass('on');
		        } else {
		            $('.jjim_SelectALLbtn_box').addClass('on');
		        }
		    } else {
		        if (isOn) {
		            $(this).removeClass('on');
		...
		$('.select_delete').click(function() {
		    ...
		    $('.jjim_SelectALLbtn_box.on').each(function() {
		        const $item = $(this).closest('.Store_jjim_item');
		        
		        // .Store_jjim_headers가 조상에 있으면 제외
		        if ($item.closest('.Store_jjim_headers').length > 0) {
		            return; // continue 역할
		        }

		        const sno = $item.data('sno');
		        if (sno !== undefined) {
		            selectedSnos.push(sno);
		        }
		    });

		    if (selectedSnos.length === 0) {
		        alert("삭제할 상품을 선택해주세요.");
		        return;
		    }

		    const storeIdx = getStoreIdxFromUrl();
		    
		    $.ajax({
		        type: 'POST',
		        url: '${pageContext.request.contextPath}/deleteJjimProduct',
		        data: JSON.stringify({ snoList: selectedSnos, storeIdx: storeIdx }),
		...
		<div class = "main_area">
			...
			<div>
				<div class = "Store_nav">
					...
					<a class = "Store_nav_category off" href="${pageContext.request.contextPath}/Storereviews?viewStoreIdx=${viewStoreIdx}">
						상점후기
						<span class = "Store_number">${store.reviewCount}</span>
					</a>
					<a class = "Store_nav_category on" href="${pageContext.request.contextPath}/Storejjim?viewStoreIdx=${viewStoreIdx}">
						찜
			...
			<div>
				<div>
					<div class = "Store_product_header">
						<div>
							찜
							<span class = "Product_number">${store.jjimCount}</span>
						</div>
					</div>
					<c:choose>
					  <c:when test="${store.jjimCount == 0}">
						<div class = "Store_product_null">찜한 상품이 없습니다.</div>
						<div class = "Store_jjim_main" style = "display:none">
					  </c:when>
					  <c:otherwise>
						<div class = "Store_product_null" style = "display:none">찜한 상품이 없습니다.</div>
						<div class = "Store_jjim_main">
					  </c:otherwise>
					</c:choose>
						<div class = "Store_jjim_main_header">
							<div class = "Store_jjim_headers">
								<div class = "jjim_SelectALLbtn_box"></div>
								<button class = "select_delete">선택삭제</button>
							</div>
							<div class = "Store_product_main_header_filter">
								<c:set var="strParam" value="${param.str != null && param.str != '' ? param.str : '최신순'}" />
								    <a class="Store_filter_this ${strParam == '최신순' ? 'on' : ''}" 
								       href="${pageContext.request.contextPath}/Storejjim?viewStoreIdx=${store.storeIdx}&str=최신순">
								       최신순
								    </a>
								    <a class="Store_filter_this ${strParam == '인기순' ? 'on' : ''}" 
								       href="${pageContext.request.contextPath}/Storejjim?viewStoreIdx=${store.storeIdx}&str=인기순">
								       인기순
								    </a>
								    <a class="Store_filter_this ${strParam == '저가순' ? 'on' : ''}" 
								       href="${pageContext.request.contextPath}/Storejjim?viewStoreIdx=${store.storeIdx}&str=저가순">
								       저가순
								    </a>
								    <a class="Store_filter_this ${strParam == '고가순' ? 'on' : ''}" 
								       href="${pageContext.request.contextPath}/Storejjim?viewStoreIdx=${store.storeIdx}&str=고가순">
								       고가순
								    </a>
							</div>
						</div>
						<div class = "Store_jjim_main_Content">
							<c:forEach var="jjim" items="${jjimList}">
							    <div class="Store_jjim_item" data-Sno = "${jjim.productIdx}">
							        <a class="Store_jjim_product_link" href="${pageContext.request.contextPath}/productInfo?productIdx=${jjim.productIdx}">
							            <div class="Store_jjim_item_selectBox">
							                <div class="jjim_SelectALLbtn_box"></div>
							            </div>
							            <div class="Store_jjim_item_img_box">
							                <div class="Store_jjim_item_img_area">
							                   <c:choose>
											      <c:when test="${empty jjim.productImg1}">
											        <img
											          src="data:image/svg+xml;base64,"
											          width="139" height="139"											          alt="상점 프로필 이미지"
											          />
											      </c:when>
											      <c:otherwise>
											        <img src="${jjim.productImg1}"
											             width="139" height="139"
											              />
											      </c:otherwise>
											    </c:choose>
							                </div>
							                <div class="Store_jjim_item_img_box_background"></div>
							            </div>
							            <div class="Store_jjim_item_imformation">
							                <div class="Store_jjim_item_imformation_box">
							                    <div class="Store_jjim_item_name">${jjim.productName}</div>
							                    <div class="Store_jjim_item_price">
							                        <div>
							                            ${jjim.price}
							                        </div>
							                    </div>
							                    <div class="Store_jjim_item_date">${jjim.saleStartDate}</div>
							                </div>
							            </div>
							        </a>
							    </div>
						...
						<div class = "Store_jjim_main_fotter"></div>
					</div>
				</div>
			</div>
		</div>
